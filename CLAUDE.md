...
- **Memory mode** (plan 013): `MctsBot.memory_mode` defaults to `MemoryMode::StoreState`. `HashOnly` is corruption-prone (collisions merge distinct states) and is diagnostic-only — do not use in production.
- **Env knobs for A/B and debugging** (read once per `get_action`): `BLOOD_MCTS_MEMORY={hash|get|store}`, `BLOOD_MCTS_WORKERS=N`, `BLOOD_MCTS_HORIZON=off`, `BLOOD_MCTS_TREE_REUSE=off`, `BLOOD_MCTS_VIRTUAL_LOSS=N`, `BLOOD_MCTS_STATS=1`.
