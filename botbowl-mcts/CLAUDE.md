...
- Three "players": `Home`, `Away`, `Chance`. Chance nodes appear whenever `state.pending_roll.is_some()` and their children are `BbAction::Chance { outcome, prob_bits }`. **Pass/fail rolls (pickup/dodge/GFI/catch — `D6PassFail`/`Sum2D6PassFail`) are first-class chance nodes in the tree (plan 018):** `available_actions` enumerates them into weighted Pass+Fail children, `score_leaf` returns `None` for the chance node (expanded, *not* scored), and its Q is the probability-weighted backprop of its outcomes. Only scatter/bounce/throw-in/block/etc. still collapse to a single deterministic `Advance` child (`roll_outcomes::enumerate`) — a documented follow-up, not done. (Plan 018 reversed plan 010 Track A.alt's optimistic fast-forward, which over-valued risky rolls — e.g. a marked-ball pickup scored as guaranteed success.)
...
- `score_leaf` is the **value function** (heuristic `leaf_score` now, NN later) and is evaluated **only at player-decision and terminal nodes** (plan 018). Pending-roll (chance) states return `None` — never scored. Mid-procedure states (no roll, no `team`, not game-over) are `available_actions → None` so `recon_mcts` marks them terminal; they are the one unexpandable exception still given `leaf_score`.
