...
- We have a target-aware `DicePolicy` hook on `GameState` (engine side, `core/dices.rs`), consulted by `get_roll_result` before the FIFO/RNG path. The `SucceedAtOrEasier { d6, sum2d6, block_dice }` variant covers the grand plan's "3+ succeeds, 4+ fails" pass/fail semantics *plus* a `KnockdownAtAdvantage` block-die policy that turns 2+ attacker-dice rolls into all-Pow. Three-outcome and FoulArmor rolls still fall through — extend the enum when a lecture demands it.
- We have a roll-by-roll stepping mode on the engine (`state.expose_rolls`, `state.pending_roll`) and a hand-rolled `Hash`/`Eq` on `GameState` ignoring `log`/`rng`/etc., so MCTS-style transposition tables can recombine equivalent positions reached via different histories.
