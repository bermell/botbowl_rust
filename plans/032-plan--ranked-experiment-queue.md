...
| **On every board below the compiled default, the engine's own `SetupLine` formation fails the engine's own `is_setup_legal`.** The clamped offsets put the front rank on the LOS *column* but at `y` outside `los_y_range`, so `line_of_scrimage` counts **0** against a required 3. Measured at 16x9/4, 18x11/6 and 22x11/8; legal at 28x17/11. Nothing enforces `is_setup_legal`, so it has never affected play — but it means a 14x7 drive starts with **nobody on the line of scrimmage**, which removes the opening block entirely and is one more reason the tier's early game is "hand the ball to a receiver" (see the row above) | a rules-design call for the tier, coupled to the kickoff row above. Fixing `Setup::setup_line`'s clamping changes the opening position of **every** 14x7 drive, so it invalidates the trained nets and every measured result here — it has to be a deliberate regeneration, not a bug-fix commit. Found while building the web UI (plan 034); pinned by `botbowl-web/server/src/view.rs::the_auto_setup_formation_is_illegal_on_clamped_boards` |
