...
## The gateless run, gen10-17 (2026-09-11 → 09-13): a rise, then a regression

Live results from the gateless loop (anchor `bbnet_14x7_gen03.onnx`, 40 paired games/gen,
`scripts/anchor_curve.py`). This is the first stretch where **each generation is generated by a
different net**, which matters for #4 below.

| gen | pts | W-D-L | rolling3 | draws | val_policy+val_value |
|----:|----:|:------|---------:|------:|---------------------:|
| 10 | 0.662 | 21-11-8 | 0.608 | 28% | — |
| 11 | 0.588 | 20-7-13 | 0.608 | 18% | 1.4657 |
| 12 | 0.713 | 25-7-8 | 0.654 | 18% | 1.3511 |
| 13 | **0.675** | 25-4-11 | **0.658** | 10% | 1.3016 |
| 14 | 0.487 | 13-13-14 | 0.625 | 33% | 1.2658 |
...
window is gen12 (window = gen10-12); the peak is gen12-13 and the break is gen14 — consistent
with diversity decaying over a few generations rather than switching off at once.

The mechanism they jointly expose is **#4**: generation is `pick_best_action`, argmax over
aggregated Q with a deterministic tie-break (`TieBreak::Hash`). Verified 2026-09-13: there is no
...
more draws. Gateless removes the ratchet that would have kept gen13 as generator once gen14 came
in weaker.

**#4's gate was failed on evidence that could not have detected this.** The entropy limb compared
gen05/06/07 (1.211 / 1.221 / 1.212) — and, as that item already says, *all three were generated by
the same frozen `bbnet_14x7_gen03.onnx`*, so it was vacuous by construction. gen10-17 is the first
...
- **The policy head learns nothing during training, in every generation.** Best-val lands at step
  2500-15000 of ~110k (gen08-17), and `val_policy` is flat *from the first eval* — gen11 ran
  1.1050 → 1.1094 over the whole 110k steps with `val_top1` pinned at 0.622-0.624. Meanwhile
  `val_value` climbs while train `value_loss` falls: ~90% of every training run is the value head
  memorising. `EPOCHS=10` is ~30 min/generation spent producing weights that are then discarded.
  Cheap fix: cut `EPOCHS`. Real question: whether warm-starting from the champion every generation
  is what freezes the policy in place (relates to #12, which chose the fine-tune on strength
  grounds before this signature was visible).
- **The `mcts-heuristic` rung was dropped 2026-09-12** (commit 5c7a651) after saturating at
...
Calibration, corrected once gen01 was on disk (2026-09-16): **the search carries most of the
conversion, not the net.** gen01's corpus — generated by the *random* seed net at the production
1000 iterations — converts **0.596** of its drives. The 0.167 quoted when this section was written
came from a 24-drive smoke test at `MCTS_ITERS=30` and is not comparable to anything; it measured
a crippled search, not a cold net.

So the span this run has to climb is 0.596 → the low 0.8s, and the useful reading of the TD/drive
column is the *delta over a random net at the same budget*, not the absolute rate. (The old gen20's
0.839 is itself pre-bugfix and only a rough target.) A generation whose corpus sits at ~0.60 has
...
| **RESOLVED 2026-09-15 — the setup formations are now board-relative, and there are four of them.** `Setup` offered one action, `SetupLine`, whose hard-coded offsets were clamped onto small boards: the front rank landed on the LOS *column* but at `y` outside `los_y_range`, so `line_of_scrimage` counted **0** against a required 3 (16x9/4, 18x11/6, 22x11/8; legal only at 28x17/11) and a 14x7 drive started with nobody on the line. `Formation` (`kickoff_procs.rs`) replaces this: `Line | Spread | Wedge | Zone`, each a pure function of `(BoardDims, TeamType)` built from `los_y_range`/wing anchors, each opening with three LOS slots, each offered as its own `SimpleAT` when the board can hold it. **This changes the opening position of every 14x7 drive** — the trained nets and every measured result above predate it and need regeneration before they are comparable. The policy head was *not* widened: the three new actions share `SetupLine`'s channel in `botbowl-nn`, so existing nets still load and evaluate (they just cannot prefer a formation; that needs a wider head and a retrain). | pinned by `kickoff_procs.rs::every_offered_formation_is_legal_on_every_board` / `offered_formations_are_distinct` and `view.rs::every_auto_setup_formation_is_legal_on_clamped_boards` |
