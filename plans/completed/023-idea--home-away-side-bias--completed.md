...
## Postscript (2026-09-01): the old corpus and champions were retired, not salvaged

The "owed" question above — was the gen-0/01/02 training pipeline itself
compromised — was answered empirically rather than left open. Two facts
made this necessary rather than optional:

1. **Every dataset-generation mode runs through the same buggy `MctsBot`.**
   `botbowl-ui/src/dataset.rs::make_mcts` shows `heuristic`, `nn`,
   `nn-value` and `pure-td` all build an `MctsBot`; there is no
   search-free generation path. So gen00's "heuristic corpus" bootstrap,
   gen01's nn-value self-play, and gen02's, were **all** generated by the
   pre-`e107f06` search, not just the nn-value shards this plan's
   "separate finding" section had suspected.
2. **A direct measurement of the champion net.** 60-game `bbnet_14x7_gen01.onnx`
   vs itself (nn-value, mirror config, 1000 iterations, seed 900000, run
   under the fixed search, commit `e7bfa8b`): **Home share 0.262 (11W-31L,
   18D, n=42 decided), z = -3.09.** Side TDs 191:232 (0.451), same
   direction. The search that produced this game is already confirmed
   equivariant (this plan, throughout), so this asymmetry can only be
   coming from the net's own learned value function — trained on targets
   the old search corrupted. Notably the **direction flipped**: the search
   bug alone produced a Home-favoring game outcome (0.667-0.727), but the
   net trained on that corrupted signal learned an Away-favoring value
   function. Not a contradiction — a network fits whatever asymmetric
   pattern is actually present in its (corrupted) training targets, which
   need not share the sign of the aggregate win-rate effect the search bug
   separately produced.

**Consequence:** `bbnet_14x7_gen00/01/02.onnx` and the `runs/loop14x7`
corpus they were trained on are retired (deleted 2026-09-01) rather than
used as a continued-training base. The training loop restarts from a
fresh gen00 bootstrap under commit `e7bfa8b` or later. This also
resolves the "separate finding" gen-0 net side-miscalibration question
above: it needn't be independently chased further, since the whole
corpus generation this net came from is being redone anyway.
...
- `runs/loop14x7/mirror.json`, `mirror.log` — the 100-game report. **Deleted 2026-09-01** along with the rest of `runs/loop14x7` and `models/bbnet_14x7_gen{00,01,02}.{onnx,pt}` — see the postscript above.
- `runs/loop14x7/gen00/shard*.jsonl` — the n=4800 in-drive null. Also deleted; the finding itself remains valid (it was a comparative null result, not a claim about absolute data quality).
